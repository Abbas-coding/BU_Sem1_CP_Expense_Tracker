...
bool registerUser() {
	...
	if (outfile.is_open())
	{
		outfile << userInfo.username << " " << userInfo.password << " " << userInfo.income << " "
...
void menu2() {
	...
	do
	{
		...
		for (int i = 0;;) {
			...
			if (key == 72 && (counter == 3 || counter == 4 || counter == 5 || counter == 6 || counter
				== 7 || counter == 8)) { //72 up Arr key, 75 left key 
				counter--;
				counter--;
			}
			if (key == 80 && (counter == 1 || counter == 2 || counter == 3 || counter == 4 || counter
				== 5 || counter == 6)) { //80 down Arr key, 77 right key 
				counter++;
				counter++;
			}
			if (key == '\r') { // '\z' carriage return OR Enter key 
...
void addExpense(string category, string fileName) {
	...
	if (outFile.is_open()) {
		outFile << newExpense.category << " " << newExpense.description << " " <<
			newExpense.amount << " "
...
void deleteExpense(const string& category, const string& fileName) {
	...
	while (getline(inFile, line)) {
		...
		Expense expense;
		ss >> expense.category >> expense.description >> expense.amount >> expense.date >>
			expense.designation;
...
void viewExpenses(const string& fileName) {
	...
	do {
		...
		for (int i = 0;;) {
			...
			if (key == 72 && (counter == 3 || counter == 4 || counter == 5 || counter == 6 || counter
				== 7 || counter == 8)) { // Up arrow key 
				counter -= 2;
			}
			if (key == 80 && (counter == 1 || counter == 2 || counter == 3 || counter == 4 || counter
				== 5 || counter == 6)) { // Down arrow key 
				counter += 2;
			}
			if (key == '\r') { // Enter key 
				log = counter;
...
void viewCategoryExpense(const string& category, const string& fileName) {
	...
	while (file >> temp.category >> temp.description >> temp.amount >> temp.date >>
		temp.designation) {
	...
	case 1:
	{
		...
		while (inFile >> tempExpense.category >> tempExpense.description >> tempExpense.amount
			...
			if (date == tempExpense.date)
			{
				...
				cout << endl;
				cout << "---------------------------------------------------------------------------------------------------------------------------------\n" <<
					"Category: " << tempExpense.category << "\t" << "Description: " <<
					tempExpense.description << "\t" << "Amount: "
		...
		while (inFile >> tempExpense.category >> tempExpense.description >> tempExpense.amount
			...
			if (amount == tempExpense.amount)
			{
				...
				cout << "--------------------------------------------------------------------------------------------------------------------------------\n" <<
					"Category: " << tempExpense.category << "\t" << "Description: " <<
					tempExpense.description << "\t" << "Amount: "
					<< tempExpense.amount << "\t\t" << "Date: " << tempExpense.date << "\t\t" <<
					"Designation: "
					<< tempExpense.designation << endl;
				cout << endl;
				cout <<
